    ...
    if ( (typeof siteID != 'undefined') && (typeof linkTrackVars != 'undefined') && (typeof linkTrackEvents != 'undefined') && (typeof linkName != 'undefined') ) {
		...
		if (linkTrackVars.indexOf('eVar22') >= 0) {
			...
			setTimeout(function () {
				...
				s.usePlugins=false;
				if (error) {
					d.trackValidationError(error.substring(0, error.length-1));
				} else {
...
d.trackValidationError = function(error, eventSerialize) {
	if(typeof error != 'string' || error.length == 0) return;
	error = error.replace(/\n/gi,'.');
	error = error.replace(/\.\./gi,'.');
	if(error[error.length-1] == '.') error = error.substring(0, error.length-1);
	error = error.replace(/\./gi,'|');
    s.usePlugins = false;
    s.linkTrackEvents = "event64";
    s.linkTrackVars = "events,list3,products";
    var override = new Object();
    override.events = (typeof eventSerialize == "undefined") ? "event64" : ("event64:" + eventSerialize);
    override.list3 = error;
    s.tl(this, 'o', 'Validation Error', override);
    s.usePlugins = true;
};

d.trackCustomLink = function(element, trackEvents, trackVars, linkName, override) {
    s.usePlugins = false;
	s.linkTrackEvents = trackEvents;
    s.linkTrackVars = trackVars;
    s.tl(element, 'o', linkName, override, 'navigate');
    s.usePlugins = true;
	s.linkTrackEvents = 'none';
    s.linkTrackVars = 'none';
};
...
d.adjustBrochureware = function(s) {
    if (s) {
		...
		//Product Engagement - get target elements		
        trackProductEngagement = function(element, linkName) {
        	var override = new Object();
        	override.events = 'event5';
        	override.prop8 = element.linkName;
        	d.trackCustomLink(element, 'event5', 'prop8', element.linkName, override);
        };
		var targetLinks = [];
		var targetLinkNames = [];
		if(!/secapp|sales/i.test(s.getPageName()) && /insurance/i.test(s.getPageName()) && /home|car/i.test(s.getPageName())) {
			var category = 'car_';
			if(/home/i.test(s.getPageName())) category = 'home_';
			for(var i = 0; i < document.links.length; i++) {
				if(/Find out more/i.test(d.getInnerText(document.links[i])) 
					&& (/home-insurance/i.test(s.getPageName()) || /button-image/i.test(document.links[i].className))
					&& !/cover$/i.test(document.links[i].href)) {
					targetLinks.push(document.links[i]);
					var entagement = '';
					if(/home-contents$|building-contents/i.test(document.links[i].href)) {
						entagement = 'B&C_';
					} else if(document.links[i].href.indexOf('home-building') > -1) {
						entagement = 'B_';
					} else if(document.links[i].href.indexOf('renters') > -1) {
						entagement = 'f&t_';
					} else if(document.links[i].href.indexOf('landlord-contents') > -1) {
						entagement = 'Landlordcontent_';
					} else if(document.links[i].href.indexOf('landlord') > -1) {
						entagement = 'Landlord_';
					} else if(document.links[i].href.indexOf('contents') > -1) {
						entagement = 'C_';
						if(document.links[i].href.indexOf('home-contents-insurance') > -1 && document.links[i].parentNode.tagName == 'P') {
							entagement = 'Rentercontent_';
						}
					} 
					document.links[i].linkName = category + entagement + 'find-out-more';
					targetLinkNames.push(category + entagement + 'find-out-more');
				}
			}
			if(targetLinks.length > 0) {
				for (var i = targetLinks.length - 1; i >= 0; i--) {
					d.attachEvent('click', function(e) {
						if(e.preventDefault) e.preventDefault();
						else if(e.returnValue) e.returnValue = false;
						else return false;
						trackProductEngagement(this, targetLinkNames[i]);
					}, targetLinks[i]);
				};
			}
		}
            ...
            if (element != null) {
                element.onclick = function trackError() {
                    ...
                    if (/You can only|Error uploading/i.test(error)) {
                        d.trackValidationError(error.replace('\n', '|'));
                    } else {
        ...
        } else if (/in:\w+:secapp:(sales:)?(motor|home)/i.test(s.pageName)) {
        	if(/payment/i.test(s.pageName)) {
        		var errorEles = d.getElementsByClassName('errors');
        		var error = '';
        		for(var i = 1; i < errorEles.length; i++) {
        			if(errorEles[i].tagName == 'DIV') error += d.getInnerText(errorEles[i]);
        		}
        		if(!error) error = d.getInnerText(errorEles[0]);
        		if(error.indexOf('. P')) error = error.replace('. P', ', p');
        		d.trackValidationError(error);
        	} else {        		
	        	var element = document.getElementById('findcar') || document.getElementById('_eventId_submit');
	        	if(element) {
	        		d.attachEvent('click', function() {
	                    setTimeout(function(){
	                    	var errorEle = document.getElementById('*.errors');
		                    var error = errorEle != null ? d.getInnerText(errorEle) : '';
		                    d.trackValidationError(error);
	                    }, 500);
	            	}, element);
	        	}
	    		d.attachEvent('readystatechange', function () {
	                if (document.readyState == 'complete') {
	                    var errorEle = document.getElementById('*.errors') || document.getElementById('errors.errors') || document.getElementById('errors');
	                    var error = errorEle != null ? d.getInnerText(errorEle) : '';
	                    d.trackValidationError(error);
	                }
	            });
        ...
        if(s.pageName.indexOf('in:ami:secapp:sales:home:quote') > -1)
        {
            if(scEventExists('event8'))
            {
                ...
                if(select)
                {
                    d.attachEvent('change', function (e) {
                        var currentElement = (ie) ? e.srcElement : e.target;
                        if (currentElement.selectedIndex != 0) {
                            s.getAndPersistValue(currentElement[currentElement.selectedIndex].text,"ami_home_previous");
                        }
                    },select);
        ...
        //  [sitecat-969]
        submit_event_evar = function(event,evar,evarValue){
            var override = new Object();
            if(event != '')
            {
                s.linkTrackEvents = "event" + event;
                override.events =  "event" + event;
            }
            s.linkTrackVars = evar;
            s.usePlugins = false;
            val="override." + evar + "=" + '\'' + evarValue +'\'';
            eval(val);
            s.tl(this, 'o', evarValue, override);
            s.usePlugins = true;
        }
        getAndforamtinnertext=function(element)
        {
            text = d.getInnerText(element);
            text = d.getTxtWhinNchar(text,26);
        ...
        if(s.getPageName() == 'in:sun:insurance:car:comprehensive')
        {
            ...
            if(links && links.length > 0)
            {
                ...
                {
                    d.attachEvent('click', function() {
                    	d.trackProductEngagement(this, 'car' + '_' + getAndforamtinnertext(this));
                    }, link[i]);
                }
            }
        }
        if(s.getPageName() == 'in:sun:insurance:car')
        {
            setTimeout(function() {
                ...
                {
                    d.attachEvent('click', function() {
                    	d.trackProductEngagement(this, 'car' + '_' + 'find-more');
                    }, button_findmore);
                }
            }, 1000);
        }
        ...
        if(/in:ami:secapp:sales:motor:quote:quote_completed/i.test(s.pageName))
        {
            ...
            for(id in ids)
            {
                ...
                if(checkbox)
                {
                    d.attachEvent('click',function(){
                        if(this.checked)
                        {
                            name = getAndforamtinnertext(this.parentElement.parentElement);
                            submit_event_evar('78','eVar69',product_name + '_' + name);
                        }
                    },checkbox);
                }
            }
            ...
            {
                if(thirdpartcheckbox.checked)
                {
                    name = getAndforamtinnertext(thirdpartcheckbox.parentElement.parentElement);
                    s.eVar69 = product_name + '_' + name;
            ...
            {
                for(link =0; link < paylinks.length ; link ++)
                {
        ...
        if(/in:sun:secapp:motor:quote:quote_completed/i.test(s.pageName))
        {
            ...
            if(paylinks && paylinks.length > 0)
            {
                ...
                {
                    d.attachEvent('click',function(){
                        name = getAndforamtinnertext(this);
                        submit_event_evar('78','eVar69',product_name + '_' + name);
                    },paylinks[link]);
                }
            }
        ...
        //[sitecat-987]
        addClickfunctionForpopover = function (productname) {
            links = document.getElementsByClassName('popover-marker');
            if (links && links.length >0) {
                for (link = 0; link < links.length; link++) {
                    if (links[link].className.indexOf('popover-help-icon') == -1 || links[link].className.indexOf('popover-help-icon') > -1) {
                        d.attachEvent('click', function () {
                        	d.trackProductEngagement(this, productname + '_' + getAndforamtinnertext(this));
                        }, links[link]);
                    }
                }
            }
        }
        addClickfunctionForlinks = function (start, end, productname) {
            links = document.links;
            if (links && end<=links.length) {
        ...
        addClickfunctionForlinksandTexts = function (context, productname, text) {
            links = document.links;
            if (links && links.length > 0) {
                for (link = 0; link < links.length; link++) {
                    txt = d.getInnerText(links[link]);
                    if (txt.indexOf(context) > -1) {
                        d.attachEvent('click', function () {
                            d.trackProductEngagement(this, productname + '_' + getAndforamtinnertext(this));
                        }, links[link]);
                    }
        ...
        addClickfunctionForlinksWithoutJump = function(productname)
        {
            links = document.links;
            if(links && links.length >0)
            for (link=0; link <links.length;link++) {
        ...
        addclickfunctionForElmentId = function(id,productname)
        {
        ...
        if (s.getPageName() == 'in:sun:insurance:home-contents') {
            addClickfunctionForpopover('home');
            addClickfunctionForlinks(32,40,'home');
        }
        if (s.getPageName() == 'in:sun:insurance:home-building') {
            addClickfunctionForpopover('home');
            addClickfunctionForlinks(28,36,'home');
        }
        if (s.getPageName() == 'in:sun:insurance:home:contents') {
            addClickfunctionForpopover('home');
            addClickfunctionForlinks(32,40,'home');
        ...
        if (s.getPageName() == 'in:sun:insurance:landlord') {
            ...
            d.attachEvent('click', function () {
                d.trackProductEngagement(this, 'landlord' + '_' + getAndforamtinnertext(this));
            }, link);

        }
        //for aami
        if (s.getPageName() == 'in:ami:car-insurance') {
            addClickfunctionForlinksandTexts('Find out more','car');
            addClickfunctionForlinksandTexts('Compare Car Insurance policies','car');
        }
        if (s.getPageName() == 'in:ami:car-insurance:comprehensive-car-insurance') {
            addClickfunctionForlinks(17,46,'car');
        }
        if(s.getPageName() == 'in:ami:car-insurance:third-party-property-car-insurance')
        {
            addClickfunctionForlinksWithoutJump('car');
        }
        if(s.getPageName() == 'in:ami:home-insurance:home-building-contents-insurance' || s.getPageName() == 'in:ami:home-insurance:home-contents-insurance')
        {
            addClickfunctionForlinksandTexts('Complete','home','completed-replacement');
            addclickfunctionForElmentId('click-me-to-slow-scroll','home');
            addclickfunctionForElmentId('click-me-to-slow-scroll-1','home');
            addClickfunctionForlinksWithoutJump('home');
        }
        if (s.getPageName() == 'in:ami:home-insurance:home-building-insurance') {
            addclickfunctionForElmentId('click-me-to-slow-scroll', 'home');
            addclickfunctionForElmentId('click-me-to-slow-scroll-1', 'home');
            addClickfunctionForlinksWithoutJump('home');
            addClickfunctionForlinksandTexts('Optional AAMI Complete Replacement Cover','home');
        }
        if(s.getPageName() == 'in:ami:home-insurance:renters-insurance')
        {
            addClickfunctionForlinksWithoutJump('home');
        }
