...
function s_doPlugins(s) {
    ...
    if(s.pageName && s.pageName == "in:apa:secapp:home:quote:completed") s.c_w('s_apia_home_bsp',true)
    ...
    // Traffic type SS
    var trafficType_ss = s.getAndPersistValue(null, 's_traffictype_ss') || '0x0x0x0';
    trafficType_ss = trafficType_ss.split('x');
    if (trafficType_ss[3] === '0') trafficType_ss[3] = s.getVisitNum();
    if (s.eVar12 !== undefined && s.eVar12 !== '') {
        if (trafficType_ss[2] != s.eVar12) trafficType_ss[1] = parseInt(trafficType_ss[1], 10) + 1;
        trafficType_ss[2] = s.eVar12;
    }
    if (scEventExists('event9')) {
        var ttEvents = s.events.split(',');
        var ttQuoteId = '';
        for(i=0;i<ttEvents.length;i++){
            if (ttEvents[i].indexOf('event9') > -1) {
                ttQuoteId = ttEvents[i].split(':')[1];
            }
        }
        var ttQuoteList = s.getAndPersistValue(null, 's_traffictype_ql') || '';
        if (ttQuoteList.indexOf(ttQuoteId) === -1) {
            ttQuoteList = ttQuoteList + ttQuoteId;
            trafficType_ss[0] = parseInt(trafficType_ss[0], 10) + 1;
            s.getAndPersistValue(ttQuoteList, 's_traffictype_ql', 0);
        }
    }
    if (scEventExists('event18') || trafficType_ss[3] != s.getVisitNum()) {
        trafficType_ss = [0,0,0,0];
        s.getAndPersistValue(' ', 's_traffictype_ql', 0);
    }
    s.getAndPersistValue(s.join(trafficType_ss, {delim: 'x'}), 's_traffictype_ss', 0);
    if (trafficType_ss[3] === s.getVisitNum() && trafficType_ss[0] >= 10 && parseInt(trafficType_ss[1], 10) > 1) s.eVar53 = 'ss';

    // Traffic type VB
    var trafficType_b = s.getAndPersistValue(null, 's_traffictype_b') || '0x0';
    trafficType_b = trafficType_b.split('x');
    if (scEventExists('event8')) trafficType_b[0] = '1';
    if (s.getVisitNum() >= 10 && trafficType_b[0] === '0') s.eVar53 = 'vb';

    // Traffic type HB
    trafficType_b[1] = (s.pageName.indexOf(':homepage') > -1) ? parseInt(trafficType_b[1], 10) +1 : 0;
    if (trafficType_b[1] >= 10) s.eVar53 = 'hb';
    s.getAndPersistValue(s.join(trafficType_b, {delim: 'x'}), 's_traffictype_b', 365);

    // Traffic type EDM
    if (s.pageName.indexOf(':edm') > -1) s.eVar53 = 'edm';
...
/*
* Plugin: getVisitNum - version 3.0c (Customised)
* Modifications: 1/10/2014 - Locally scoping ‘d’ var in ‘s.endof’ function.
*/
...
+"var t=new Date,d;t.setHours(0);t.setMinutes(0);t.setSeconds(0);if(x=="
